...
ToonShader::ToonShader(float shininessExponent,
...
Color ToonShader::shade(Ray * ray) const {
    ...
    for (unsigned int i = 0; i < lights.size(); ++i) {
        ...
        if (diffuseLight <= EPSILON) {
            ...
            specularLight = std::pow( std::max( dotProduct(normalVector, hVec), 0.0f ), shininessExponent );
        }

        float edge = (dotProduct(normalVector, hVec) > 0.3f) ? 1 : 0;
        float light = edge * (diffuseLight + specularLight);
        illuminationColor += Color(light,light,light);
    }
